...
// Update verifySession to expect new structure
async function verifySession(session_id: string): Promise<{ status: number; message: string; data: StripeSessionData }> {
    const res = await getRequest<{ status: number; message: string; data: StripeSessionData }>(`/billing/session/${session_id}`)
...
function StripeSuccessPageInner() {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-[100vh] py-12 px-4">
            <Card className="w-full max-w-xl border-0 shadow">
                ...
                <CardContent className="flex flex-col gap-6 mt-2">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-4 text-sm mt-2">
                            <span><span className="font-medium">Transaction ID:</span> {transactionId}</span>
